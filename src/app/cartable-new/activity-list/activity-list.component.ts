...
export class ActivityListComponent extends BaseComponent implements OnInit {
  ...
  getMenuItems(activity: Activity): MenuItem[] {
    ...
    this.translate.get('action').subscribe(trans => {
      menuItems.push(
        { label: trans.view, icon: 'fa fa-eye', command: () => { this.onView(activity); } }
      );
  ...
  onDelete(activities: Activity[]) {
    this.delete.emit(activities);
    this.selectedActivities = [];
  }

  onRestore(activities: Activity[]) {
    this.restore.emit(activities);
    this.selectedActivities = [];
  }

  onArchive(activities: Activity[]) {
    this.archive.emit(activities);
    this.selectedActivities = [];
  }
